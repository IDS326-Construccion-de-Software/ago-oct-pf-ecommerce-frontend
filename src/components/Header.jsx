...
import ShoppingCart from "../pages/ShoppingCart.jsx"; // Importar el nuevo componente
...
export default function Header() {
  ...
  const [cartItems, setCartItems] = useState([]); // aquí podrías llenar productos de prueba
  ...
  return (
    <>
            ...
            <ul className="cats">
              {CATEGORIES.map((c) => (
                <li key={c} className="cat-item">
                  <NavLink to={`/categoria/${slug(c)}`} className={({ isActive }) => `cat-link ${isActive ? "active" : ""}`}>
                    {c}
                  </NavLink>
                </li>
              ))}
      ...
      {/* Drawer del carrito */}
      <ShoppingCart isOpen={cartOpen} onClose={() => setCartOpen(false)} items={cartItems} />
    </>
  );
}
